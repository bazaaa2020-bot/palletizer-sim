...
function curtain3(w,h,y0){const g=new THREE.Group(),caps=[];
 [-w/2,w/2].forEach(x=>{const c=box3(0.05,h+0.10,0.07,COL3.joint);c.position.set(x,y0+h/2,0);g.add(c);
  const cap=box3(0.055,0.035,0.075,COL3.red);cap.position.set(x,y0+h+0.06,0);g.add(cap);caps.push(cap);});
 const pl=new THREE.Mesh(new THREE.PlaneGeometry(w,h),mat(COL3.red,{transparent:true,opacity:0.10,side:THREE.DoubleSide}));
 pl.position.y=y0+h/2;g.add(pl);
 const pts=[];for(let y=y0+0.06;y<y0+h;y+=0.12)pts.push(-w/2,y,0,w/2,y,0);
...
function build3D(){
 ...
 const addOp=(hit,w)=>{if(!hit)return null;const o={s:hit.s,w,x:hit.x,y:hit.y,ang:hit.ang};ops[hit.w].push(o);return o;};
 ...
 if(D.safety==='fence'){
  ...
  Object.keys(slotOp).forEach(id=>{const o=slotOp[id];const g=curtain3(o.w*mm,CH,FENCE_GAP);
   g.position.set(o.x*mm,0,o.y*mm);g.rotation.y=o.ang;R.add(g);V3.gates[id]=g;});
...
function render3D(){
 ...
 S.st.forEach(s=>{const st=V3.stations[s.id];if(!st)return;const sig=`${s.present}|${s.layer}|${s.placed.length}|${s.parity}|${s.sheetLayers.length}|${s.complete}`;if(V3.cache[s.id]!==sig){V3.cache[s.id]=sig;buildStation3(s);}
  ...
  setCurtain(V3.gates[s.id],s.phase==='out'||(released(s)&&c.exch.out!=='conveyor')?COL3.off:COL3.red);});
 S.mags.forEach(m=>{const st=V3.mags[m.id];if(!st)return;const hh=Math.max(0.004,m.sheets*0.004);st.scale.y=hh;st.position.y=pal.h*mm+hh/2;setCurtain(V3.gates[m.id],m.loading?COL3.off:COL3.red);});
