...
function renderCfg(){
 ...
 ${Object.values(D.pats).map(p=>`<div class="check"><div>${p.name}<small>${p.stability}${p.loss>0?`; перевязка стоит ${p.loss} шт. в слое`:''}; ${p.groupsPerPallet} ходов${p.sheets?` + ${p.sheets} листов`:''} на паллету</small></div><span class="badge ${p.interlock?'ok':'warn'}">${p.n} в слое · ${p.layers} слоёв · ${p.total} шт.</span></div>`).join('')}`;
...
function renderPlan(P){
 ...
 $('cfgPlan').innerHTML=`<p class="note">Как в оценке производительности паллетайзера: циклов на паллету = ходов с коробками (группы в слое × слоёв) + листы${c.exch.in==='robot'?' + паллета':''}; требуемые циклы/мин = циклы на паллету ÷ время, за которое приходит паллета коробок. Робот даёт <b>${f1(P.capCpm)} циклов/мин</b>${c.robots>1?' (два робота)':''} при ${c.grip.pick} коробк${c.grip.pick===1?'е':'ах'} за захват.</p>
...
const STRUCT=/^(robot|robots|custom\.cls|grip\.type|grip\.cup|grip\.pick|stations|magazines|patternMode|pallet|safety|sheet\.mode|exch\.(out|in|sheetsBy|auto)|conveyors\.\d+\.(type|feed|box))$/;
...
function runOptimizer(){const out=$('optOut');out.innerHTML='<p class="note">Считаю варианты…</p>';setTimeout(()=>{OPT=optimize(CFG);OPT.opt=deep(CFG.opt);const b=OPT.best;
 ...
 const line=x=>`${x.nR>1?'2 × ':''}${x.r.name}, по ${x.k} кор. за захват, ${x.st} станц${x.st===1?'ия':'ии'}${x.nR>1?' у каждого':''} → ${f0(x.D.bottleneck)} кор/ч, ограничивает ${x.D.bnName}; досягаемость ${f0(x.D.rReq)}/${f0(x.r.reach)} мм, нагрузка ${f0(x.D.util*100)} %, защита — ${x.D.safety==='fence'?'ограждение':'сканеры'}; индекс стоимости ${f1(x.cost)}`;
...
function renderArch(){
 ...
 $('archCards').innerHTML=[['Роботы',`${nR} × ${D.rob.name}`,`нагрузка ${f0(D.util*100)} %, досягаемость ${f0((1-D.reachMargin)*100)} %`],['Захват',`${GRIPPERS[c.grip.type].name}`,`${c.grip.pick} кор. за ход, ${f0(D.grip.Fth)}/${f0(D.grip.Fcap)} Н`],['Защита',D.safety==='fence'?'Ограждение + завесы':'Сканеры + снижение скорости',D.safety==='fence'?`S завесы ${f0(D.Slc)} мм`:`поле ${f0(D.Ssc)} мм`],['Сигналы ПЛК',`${D.io.DI} DI / ${D.io.DO} DO`,`${D.io.BUS} объектов по шине`],['Цепь безопасности',`${D.io.SI} двухкан. входов`,`${D.io.SO} безопасных выходов`],['Производительность',`${f0(D.bottleneck)} кор/ч`,`${D.cyclesPerPallet} циклов на паллету; ограничивает ${D.bnName}`]].map(x=>`<div class="card"><span>${x[0]}</span><b>${x[1]}</b><span>${x[2]}</span></div>`).join('');
...
function robotTick(r,dt){
 ...
 switch(r.step){
  ...
  case 'grip':r.dwell-=dt;if(r.dwell<=0){const cv=S.conv[r.job.conv],have=cv.boxes.filter(x=>x.p>=1-(r.job.g-1)*gapOf(r.job.conv)-0.01).length;
   ...
   cv.boxes.sort((a,x)=>x.p-a.p);cv.boxes.splice(0,r.job.g);r.carry=r.job.g;r.carryKind='box';r.carryBi=DD.LY.convs[r.job.conv].bi;r.step='toPlace';r.zt=zTravel(r);}break;
