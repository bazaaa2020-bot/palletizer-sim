...
// В плотных таблицах в списке показываем короткое имя, полное — подсказкой при наведении.
const shortName=n=>String(n).split(' (')[0].split(': ')[0];
const sel=(k,opts,v,dis)=>{const cur=opts.find(o=>String(o[0])===String(v));
 return `<select data-k="${k}" ${dis?'disabled':''} title="${cur?String(cur[2]||cur[1]).replace(/"/g,'&quot;'):''}">`
  +opts.map(o=>`<option value="${o[0]}" ${String(o[0])===String(v)?'selected':''}>${o[1]}</option>`).join('')+'</select>';};
function renderCfg(){
 const c=normalize(CFG),rob=robotOf(c),D=derive(c,true),G=D.grip,vac=GRIPPERS[c.grip.type].vac,ex=c.exch,sh=c.sheet;
 $('cfgRobot').innerHTML=`<div class="field"><label>Количество роботов</label>${sel('robots',[[1,'1 робот'],[2,'2 робота']],c.robots)}<small>Параллельные модули: второй робот получает свой конвейер и свои станции, ячейка общая</small></div>
 ...
 <div class="field"><label>Скорость робота, %</label>${num('speedPct',c.speedPct,5,10,100)}<small>Влияет на такт, ускорение груза и требуемую силу захвата (a = v / 0,25 с)</small></div>
 ...
 <div class="field"><label>Скорость подвода, мм/с</label>${num('motion.vPlace',c.motion.vPlace,10,20,400)}<small>Последние миллиметры к коробке и к слою робот идёт медленно, 50–150 мм/с — именно это отличает укладку от броска</small></div>
 ...
 <div class="field"><label>Скорость вертикали, мм/с</label>${num('motion.vZ',c.motion.vZ,50,100,2000)}<small>Быстрый участок подъёма и опускания</small></div>
 <div class="field"><label>Нарастание ускорения, с</label>${num('motion.tJerk',c.motion.tJerk,.05,0,1)}<small>Ограничение рывка (S-образный профиль): чем больше, тем плавнее разгон и тем меньше тара ведёт в захвате</small></div>
 ...
 $('cfgGrip').innerHTML=`<div class="field"><label>Тип захвата</label>${sel('grip.type',Object.entries(GRIPPERS).map(([k,v])=>[k,shortName(v.name),v.name]),c.grip.type)}</div>
 ...
 <div class="field"><label>Масса захвата, кг</label>${num('grip.mass',c.grip.mass,.5,0,80)}<small>0 — считать автоматически; расчётная масса ${f1(G.mass)} кг</small></div>
 <div class="field"><label>Высота захвата, мм</label>${num('gripH',c.gripH,10,50,600)}</div>
 ...
 <div class="field"><label>Высота стопы с паллетой, мм</label>${num('maxStack',c.maxStack,50,300,2400)}</div>
 ...
 <div class="field"><label>Схема укладки</label>${sel('patternMode',[['auto','Авто'],['interlock','Перекрёстная'],['column','Колонная']],c.patternMode)}<small>Авто — максимум коробок с перевязкой, если она возможна; перекрёстная — перевязка обязательна; колонная — простая сетка</small></div>
 ...
 <div class="row tara">${sel(`boxes.${i}.shape`,Object.entries(SHAPES).map(([k,v])=>[k,shortName(v.name),v.name]),b.shape)}${sel(`boxes.${i}.mat`,Object.entries(MATS).map(([k,v])=>[k,shortName(v.name),v.name]),b.mat)}<span class="fitn ${gripFit(b,c.grip.type).s==='ok'?'':gripFit(b,c.grip.type).s}">${(()=>{const fi=gripFit(b,c.grip.type);return fi.s==='ok'?`${GRIPPERS[c.grip.type].name.toLowerCase()} подходит`:`${fi.n}. Просится: ${gripBest(b).map(t=>GRIPPERS[t].name.toLowerCase()).join(' или ')||'подхват снизу'}`;})()}</span></div>`).join('')}
 ...
 $('cfgMix').innerHTML=`<div class="field"><label>Смешанная паллета</label>${sel('mix.on',[['false','Нет, один артикул'],['true','Да, по рецепту']],c.mix.on)}<small>Ярусы идут снизу вверх; у каждого яруса свой артикул, своя схема слоя и свой конвейер</small></div>
 ${c.mix.on?`<div class="field"><label>Название рецепта</label><input type="text" data-k="mix.name" value="${(c.mix.name||'').replace(/"/g,'&quot;')}"></div>
 <div class="row mix h"><span>№</span><span>Артикул</span><span>Слоёв</span><span>Лист под ярусом</span><span></span></div>
 ${c.mix.rows.map((r,i)=>`<div class="row mix"><span class="h">${i+1}</span>${sel(`mix.rows.${i}.box`,c.boxes.map((b,j)=>[j,b.name]),r.box)}${num(`mix.rows.${i}.layers`,r.layers,1,1,40)}${sel(`mix.rows.${i}.sheet`,[['false','нет'],['true','да']],r.sheet)}<button data-act="delMixRow" data-i="${i}" ${c.mix.rows.length<2?'disabled':''}>×</button></div>`).join('')}
 <div class="btns"><button data-act="addMixRow" ${c.mix.rows.length>=8?'disabled':''}>Добавить ярус</button></div>
 ${(()=>{const M=D.mix;if(!M||!M.ok)return'';
  return `<div class="check"><div>Паллета по рецепту<small>${M.rows.map(r=>`${r.b.name} × ${r.n} сл. по ${r.pat.n} шт.`).join('; ')}${M.sheets?`; разделительных листов ${M.sheets}`:''}</small></div><span class="badge ${M.warn.length?'warn':'ok'}">${M.layers} слоёв · ${M.total} шт. · ${f0(M.mass)} кг · ${f0(M.height)} мм</span></div>
  <div class="check"><div>Ходов робота на паллету<small>считается по каждому слою своей схемой: ${f1(M.kEff)} единиц тары за ход в среднем</small></div><span class="badge ok">${M.groups}${M.sheets?` + ${M.sheets} листов`:''}</span></div>
  ${M.warn.map(x=>`<p class="note" style="color:var(--warnfg)">${x}</p>`).join('')}`;})()}`
 :'<p class="note">Моно-паллета: станция набирает один артикул со своего конвейера. Рецепт нужен, когда на одну паллету собирают заказ из нескольких артикулов — тогда у каждого яруса своя схема укладки и свой конвейер подачи.</p>'}`;
 ...
 ${c.conveyors.map((cv,i)=>`<div class="row conv">${sel(`conveyors.${i}.type`,Object.entries(CONV_TYPES).map(([k,v])=>[k,shortName(v.name),v.name]),cv.type)}${num(`conveyors.${i}.len`,cv.len,.5,1,12)}${num(`conveyors.${i}.speed`,cv.speed,1,3,60)}${sel(`conveyors.${i}.accum`,[['true','да'],['false','нет']],cv.accum,cv.type==='mdr')}${sel(`conveyors.${i}.box`,c.boxes.map((b,j)=>[j,b.name]),cv.box)}<button data-act="delConv" data-i="${i}" ${c.conveyors.length<2?'disabled':''}>×</button></div>
 ...
 <div class="row base">${sel(`conveyors.${i}.infeed`,Object.entries(INFEED).map(([k,v])=>[k,shortName(v.name),v.name]),cv.infeed)}${sel(`conveyors.${i}.align`,Object.entries(ALIGN).map(([k,v])=>[k,shortName(v.name),v.name]),cv.align)}<span class="fitn ${(()=>{const x=D.base[i];return x.ok?'':visionOK(c.vision.mode,x.need)?'warn':'bad';})()}">${(()=>{const x=D.base[i];
 ...
 <div class="field"><label>Техническое зрение</label>${sel('vision.mode',Object.entries(VISION).map(([k,v])=>[k,shortName(v.name),v.name]),c.vision.mode)}<small>${D.visNeed==='none'?'По базированию камера не требуется — направляющие выводят тару в допуск захвата':`Требуется минимум: ${VISION[D.visNeed].name}`}${c.vision.mode!=='none'?`; кадр добавляет ${f2(D.tVision)} с к такту`:''}</small></div>
 ...
 ${ex.out!=='conveyor'?`<div class="field"><label>Режим обмена в симуляции</label>${sel('exch.auto',[['true','Автоматически'],['false','По команде с пульта']],ex.auto)}<small>Автоматически — тележка или человек приезжают сами через заданное время; иначе обмен вызывается кнопкой на пульте</small></div>
 ...
 $('cfgSafety').innerHTML=`<div class="field"><label>Защита</label>${sel('safety',[['auto','Авто по типу робота'],['fence','Ограждение и завесы'],['cobot','Сканеры без ограждения']],c.safety,rob.cls!=='cobot')}<small>${rob.cls!=='cobot'?'Промышленный робот — только ограждение':'Кобот: допустима работа без ограждения при оценке риска по ISO/TS 15066'}</small></div>
 ...
 <div class="field"><label>Радиус расстановки, мм</label>${num('slotR',c.slotR,25,600,3000)}<small>Фактически ${f0(D.LY.R)} мм${D.LY.R>c.slotR?' (увеличен, чтобы позиции не перекрывались)':''}</small></div>
...
function renderChecks(D){
 ...
 {const bad=D.base.filter(x=>!x.ok&&!visionOK(c.vision.mode,x.need)),vis=D.base.filter(x=>!x.ok&&visionOK(c.vision.mode,x.need));
  ...
  rows.push(['Техническое зрение',bd(D.visOK?'ok':'bad',c.vision.mode==='none'?'не используется':VISION[c.vision.mode].name.split(':')[0]),
   D.visNeed==='none'?'По базированию не требуется: направляющие выводят тару в допуск':`Требуется минимум ${VISION[D.visNeed].name}; ${D.visOK?'выбранное подходит':'выбранное не закрывает задачу'}`]);}
...
const STRUCT=/^(robot|robots|custom\.cls|grip\.(type|cup|pick|pitch|tShift)|boxes\.\d+\.(shape|mat)|vision\.mode|mix\.(on|name)|mix\.rows\.\d+\.(box|layers|sheet)|conveyors\.\d+\.(infeed|align)|stations|magazines|patternMode|pallet|safety|sheet\.mode|exch\.(out|in|sheetsBy|auto)|conveyors\.\d+\.(type|feed|box))$/;
...
$('tab-cfg').addEventListener('click',e=>{const act=e.target.dataset.act;if(!act)return;const i=+e.target.dataset.i;
 ...
 if(act==='addMixRow')CFG.mix.rows.push({box:Math.min(CFG.mix.rows.length,CFG.boxes.length-1),layers:2,sheet:true});
 if(act==='delMixRow')CFG.mix.rows.splice(i,1);
...
function buildSim(){
 ...
 S={packml:'Stopped',why:'Требуется сброс после включения питания',estop:false,door:'closed',lock:true,accessReq:false,safetyStop:false,autoResume:false,resumeTo:'Idle',
  ...
  st:DD.stations.map(s=>Object.assign(newPal(c.exch.in!=='robot'),{id:s.id,robot:s.robot,slot:s,bi:s.bi,pat:s.pat,rec:s.rec,layers:s.rec.length,mix:!!(DD.mix&&DD.mix.ok)})),
 ...
 plcInit(); S.st.forEach(s=>{if(s.present&&S.mags.length&&(c.sheet.mode==='bottom'||needSheetAt(s,0)))s.needSheet=true;});
...
// Слой паллеты знает свой артикул и свою схему: для моно-паллеты рецепт из одинаковых слоёв.
function recAt(s,lay){return s.rec[clamp(lay,0,s.rec.length-1)];}
function patOf(s){return recAt(s,s.layer).pat;}
function biOf(s){return recAt(s,s.layer).bi;}
function recH(s,lay){let z=0;for(let j=0;j<Math.min(lay,s.rec.length);j++)z+=s.rec[j].b.h;return z;}
function recMass(s){return s.rec.reduce((a,r)=>a+r.pat.n*r.b.m,0)+DD.pal.m;}
function recTotal(s){return s.rec.reduce((a,r)=>a+r.pat.n,0);}
// Конвейер, на котором лежит нужный артикул: для смешанной паллеты он меняется от слоя к слою.
function convFor(ri,bi){const cs=DD.LY.robots[ri].convs;for(const ci of cs)if(CFG.conveyors[ci].box===bi)return ci;return -1;}
function convOf(s){return s.mix?convFor(s.robot,biOf(s)):s.slot.conv;}
function needSheetAt(s,lay){const r=s.rec[lay];return !!(r&&r.sheet);}
function curCells(s){return s.parity?patOf(s).cellsB:patOf(s).cells;}
function curGroups(s){return s.parity?patOf(s).groupsB:patOf(s).groupsA;}
...
function findJob(ri){
 ...
 for(const ci of convs){const have=boxesAtEnd(ci);if(!have)continue;for(const s of sts.filter(s=>convOf(s)===ci)){if(!avail(s))continue;if(s.needSheet){const m=mags.find(m=>m.sheets>0);if(m)return{kind:'sheet',st:s,mag:m};continue;}const g=curGroups(s)[s.gi];if(have>=g.length)return{kind:'box',conv:ci,st:s,g:g.length,group:g};}}
...
function zSafe(r,hLoad){const c=CFG,pal=DD.pal;let z=0;
 ...
 S.st.filter(s=>s.robot===r.i).forEach(s=>{if(!s.present)return;z=Math.max(z,pal.h+recH(s,s.layer)+(s.placed.length?recAt(s,s.layer).b.h:0)+s.sheetLayers.length*4);});
...
function zTravel(r){const c=CFG,pal=DD.pal;let z=c.convH+400;S.st.filter(s=>s.robot===r.i).forEach(s=>{z=Math.max(z,pal.h+recH(s,s.rec.length)+s.rec.length*4+300);});return z;}
function robotTick(r,dt){
 ...
 const s=r.job?r.job.st:null;const b=s?recAt(s,s.layer).b:DD.heaviest;
 switch(r.step){
  ...
  case 'place':r.dwell-=dt;if(r.dwell<=0){r.carryKind=null;r.carry=0;s.count+=r.job.g;S.stats.placed+=r.job.g;S.flags.typesPlaced.add(s.bi);r.job.group.forEach(i=>s.placed.push(i));s.gi++;
   if(s.gi>=curGroups(s).length){s.layer++;s.last={cells:curCells(s),parity:s.parity};s.placed=[];s.gi=0;S.tasks.t1||done('t1');
    if(s.layer>=s.layers){s.complete=true;S.flags.palletDone=true;S.stats.pallets++;log(`Паллета ${s.id} готова: ${s.count} коробок${s.mix?` (${[...new Set(s.rec.map(r=>r.bi))].map(bi=>`${boxOf(c,bi).name} — ${s.rec.filter(r=>r.bi===bi).reduce((a,r)=>a+r.pat.n,0)}`).join(', ')})`:''}, ${s.sheets} прокладок, ${f0(recMass(s))} кг${c.exch.out!=='conveyor'?` — лампа H${s.id} «освобождено»`:''}`);if(c.exch.out==='conveyor'){s.phase='out';log(`Станция ${s.id}: конвейер паллет ВКЛ, завеса S6${s.id} в мьютинге на выезд`);}}
    else{s.parity=patOf(s).interlock?s.layer%2:0;if(S.mags.length&&(needSheetAt(s,s.layer)||sheetAfterLayer(c.sheet,s.layer,s.layers)))s.needSheet=true;}}
  ...
  case 'placePallet':r.dwell-=dt;if(r.dwell<=0){r.carryKind=null;Object.assign(s,newPal(true));if(S.mags.length&&(c.sheet.mode==='bottom'||needSheetAt(s,0)))s.needSheet=true;S.stats.exch++;S.stats.setPallets++;log(`Станция ${s.id}: робот поставил пустую паллету из стопки (B${s.id}=1, осталось ${r.job.ps.n})`);S.tasks.t15||done('t15');r.step='upStation';}break;
...
function agentTick(a,dt){
 ...
 switch(a.phase){
  ...
  case 'set':a.t-=dt;if(a.t<=0){Object.assign(s,newPal(true),{agent:a});if(S.mags.length&&(c.sheet.mode==='bottom'||needSheetAt(s,0)))s.needSheet=true;a.carry=null;log(`Пустая паллета на станции ${s.id} (B${s.id}=1)`);if(S.flags.palletRemoved&&!S.tasks.t5)done('t5');a.phase='back2';a.leg=0;}break;
...
function renderGraf(){
 ...
 $('gcMode').innerHTML=`<div class="field"><label>Чья логика исполняется</label>${sel('plc.mode',[['ref','Эталонная'],['user','Моя программа']],c.plc.mode)}<small>${user?'Ячейка работает по вашей программе — ошибки в ней сразу видны на пульте':'Эталон можно скопировать к себе и править'}</small></div>
...
function palSvg(s){const pal=DD.pal,sc=GG.sc,b=recAt(s,s.layer).b,col=boxColor(biOf(s));if(!s.present)return'';
 ...
 let out=rotG(sl,o);out+=`<text x="${px}" y="${py+4}" font-size="11" text-anchor="middle" fill="var(--ink)" font-weight="600" stroke="var(--panel)" stroke-width="3" paint-order="stroke">${s.id} ${s.layer}/${s.layers} · ${s.count}</text>`;
